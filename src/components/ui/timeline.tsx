...
export const Timeline = ({
  ...
  activeLineGradient = 'linear-gradient(to bottom, #A0845C, #B89B74, transparent)',
  className
}: TimelineProps) => {
        ...
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              {/* Dot */}
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-[#F5F5F7] flex items-center justify-center">
                <motion.div
                  className="h-4 w-4 rounded-full border-2 border-[#A0845C] bg-[#F5F5F7]"
              ...
              {/* Year text */}
              <h3 className="hidden md:block text-xl md:pl-20 md:text-6xl font-bold text-[#1D1D1F]">
                {item.title}
              </h3>
            </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-3xl mb-4 text-left font-bold text-[#1D1D1F]">
                {item.title}
              </h3>
              {item.content}
            </div>
